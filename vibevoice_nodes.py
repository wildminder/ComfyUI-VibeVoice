...
class VibeVoiceTTSNode:
        ...
        return {
            "required": {
                ...
                "text": ("STRING", {
                    ...
                    "tooltip": "The script for the conversation. Use 'Speaker 1:', 'Speaker 2:', etc. to assign lines to different voices. For single speaker, just type text without speaker prefix. Each speaker line should be on a new line."
                }),
            ...
            "optional": {
                "speaker_1_voice": ("AUDIO", {"tooltip": "Reference audio for 'Speaker 1' in the script. Required if using speaker prefixes or single speaker mode."}),
    ...
    def preprocess_text(self, text):
        """Preprocess text to handle single speaker mode and remove empty lines/paragraphs"""
        lines = text.strip().split('\n')
        processed_lines = []
        current_speaker = None
        current_text = []
        
        # Check if any line has speaker prefix
        has_speaker_prefix = any(re.match(r'^\s*Speaker\s+\d+:', line, re.IGNORECASE) for line in lines if line.strip())
        ...
        for line in lines:
            ...
            if has_speaker_prefix:
                # Handle speaker prefix format
                speaker_match = re.match(r'^\s*Speaker\s+(\d+):\s*(.*)$', line, re.IGNORECASE)
                if speaker_match:
                    if current_speaker is not None and current_text:
                        processed_lines.append((current_speaker, ' '.join(current_text)))
                    current_speaker = int(speaker_match.group(1)) - 1  # Convert to 0-based
                    current_text = [speaker_match.group(2).strip()]
                else:
                    current_text.append(line)
            else:
                # Single speaker mode - all text goes to speaker 0
    ...
    def generate_audio(self, model_name, text, attention_mode, cfg_scale, inference_steps, seed, do_sample, temperature, top_p, top_k, quantize_llm_4bit, force_offload, **kwargs):
        ...
        parsed_lines_0_based = self.preprocess_text(text)
        if not parsed_lines_0_based:
            raise ValueError("Script is empty or contains only whitespace.")
        
        # Extract unique speaker IDs (1-based for voice lookup)
        speaker_ids_1_based = list(set([speaker_id + 1 for speaker_id, _ in parsed_lines_0_based]))
        speaker_ids_1_based.sort()
        
        # Create full script with speaker prefixes for the model
        full_script_lines = []
        for speaker_id, line_text in parsed_lines_0_based:
            full_script_lines.append(f"Speaker {speaker_id + 1}: {line_text}")
